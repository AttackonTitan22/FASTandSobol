...
for n in range(6,11):
    ...
    print(f"===================样本数为{2**n+1}的NEP实验开始===================")

    # Generate samples
    param_values,s=paper_FAST_sample.sample(problem, 2**n+2)
    ...
    for i in range(epoch):
        ...
        # Run model again
        param_values = constant_FAST_sample.sample(problem, 2**n+2, S=s)
...
# 将本次数据存入文件中
csv_file = open('../FAST_TAE/groupD_TAE.csv', 'w', newline='')
